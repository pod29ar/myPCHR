$(document).ready(function () {
	...
	function manipulateMeal() {
		var total = mealData.total,
			surcharge = mealData.surcharge;
		total += surcharge;
		mealData.total = total;
	}

	function postRequest() {
		var url = host + 'data/ordered',
			data = {
				'orders' : JSON.stringify(mealData),
				'flag' : 'meal'
			};
	...
	confirmMeal.click(function (e) {
		e.preventDefault();
		var checker = levelOne.find('.lbpe:visible'),
			check = checker.filter(function () {
				return $(this).html() === 'ADD';
			});
		if (check.length > 0) {
			alert('Complete your meal selection first!');
		} else {
			manipulateMeal();
			postRequest();
		}
